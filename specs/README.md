...
| [005](./005-twists-submission/) | `twists-submission` | `in-progress` | 002, 003 | 3 endpoints autenticados (`POST/DELETE /twists/*`, `GET /me/twists`), advisory lock para quota race-safe, soft delete sin liberar quota, Idempotency-Key requerido, integración con cycle.state. |
...
| [007](./007-voting/) | `voting` | `spec-done` | 002, 003, 005, 006 | Vote-feed con sort estable per-user (anti refresh-gaming), cursor pagination, UNIQUE constraint + ON CONFLICT como idempotencia natural, advisory lock para quota race, optimistic UI. |

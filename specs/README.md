...
| [003](./003-cycle-fsm/) | `cycle-fsm` | `done` | 001 | FSM 8 estados, advisory locks, idempotency UNIQUE, replace stub `/internal/transition` con executor real, kill-switch, watchdog, 4 cron workflows habilitados, stubs de side-effects para 006/008. |
| [004](./004-chapters-content/) | `chapters-content` | `done` | 001, 003 | 3 endpoints read-only (`/chapters/today`, `/chapters/{id}`, `/seasons/{slug}`), bible redaction allowlist, windows computados server-side, ETag + Cache-Control + 304, R2 público (sin proxy), kill-switch + no-season handling. |

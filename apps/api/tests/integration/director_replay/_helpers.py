"""Sync helpers for the director-replay integration tests (T-011).

Mirrors the shape of ``tests/integration/director_filter/_helpers.py``
but keeps the slugs / table prefix distinct so the two suites can run
in parallel without colliding on ``uniq_one_active_season``.
"""
...
async def make_season_and_chapter(
    ...
    """Insert an inactive season + chapter; return ``(season_id,
